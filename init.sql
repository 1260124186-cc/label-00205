...
CREATE TABLE IF NOT EXISTS sc_tenant_quotas (
    ...
    max_training_concurrent INT DEFAULT 2 COMMENT '最大训练并发数',
...
-- ============================================================
-- 租户强隔离迁移：添加 tenant_id 索引和配额扩展
-- ============================================================

-- sc_bolt_data: 添加 tenant_id 索引
SET @dbname = DATABASE();
SET @tablename = 'sc_bolt_data';
SET @indexname = 'idx_tenant';
SET @preparedStatement = (SELECT IF(
  (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS
...
-- sc_tenant_quotas: 添加 max_training_concurrent 列
SET @tablename = 'sc_tenant_quotas';
SET @columnname = 'max_training_concurrent';
SET @preparedStatement = (SELECT IF(
  (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
   WHERE table_name = @tablename AND table_schema = @dbname AND column_name = @columnname) > 0,
  'SELECT 1',
  CONCAT('ALTER TABLE ', @tablename, ' ADD COLUMN ', @columnname, ' INT DEFAULT 2 COMMENT ''最大训练并发数'' AFTER max_org_nodes')
));
PREPARE alterIfNotExists FROM @preparedStatement;
EXECUTE alterIfNotExists;
DEALLOCATE PREPARE alterIfNotExists;

-- sc_alert_rules: 添加 tenant_id 索引
SET @tablename = 'sc_alert_rules';
SET @indexname = 'idx_tenant';
SET @preparedStatement = (SELECT IF(
  (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS
   WHERE table_name = @tablename AND table_schema = @dbname AND index_name = @indexname) > 0,
...
-- sc_alert_events: 添加 tenant_id 索引
SET @tablename = 'sc_alert_events';
SET @indexname = 'idx_tenant';
SET @preparedStatement = (SELECT IF(
  (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS
   WHERE table_name = @tablename AND table_schema = @dbname AND index_name = @indexname) > 0,
...
-- sc_work_orders: 添加 tenant_id 索引
SET @tablename = 'sc_work_orders';
SET @indexname = 'idx_tenant';
SET @preparedStatement = (SELECT IF(
  (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS
   WHERE table_name = @tablename AND table_schema = @dbname AND index_name = @indexname) > 0,
...
-- sc_model_versions: 添加 tenant_id 索引
SET @tablename = 'sc_model_versions';
SET @indexname = 'idx_tenant';
SET @preparedStatement = (SELECT IF(
  (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS
   WHERE table_name = @tablename AND table_schema = @dbname AND index_name = @indexname) > 0,
...
-- sc_training_logs: 添加 tenant_id 索引
SET @tablename = 'sc_training_logs';
SET @indexname = 'idx_tenant';
SET @preparedStatement = (SELECT IF(
  (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS
   WHERE table_name = @tablename AND table_schema = @dbname AND index_name = @indexname) > 0,
...
WHERE INDEX_NAME = 'idx_tenant' AND TABLE_SCHEMA = DATABASE();
